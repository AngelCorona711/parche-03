...
$f3->route('POST /Registro',
	function($f3) {
		...
		/////////////
		$R = array_key_exists('uname',mysql_real_escape_string($jsB)) && array_key_exists('email',mysql_real_escape_string($jsB)) && array_key_exists('password',mysql_real_escape_string($jsB));
		...
		try {
			//$query = $db->prepare("insert into Usuario values(null,$username,$email,$password)");
			//$query->execute(array($username,$email,$password));
			$R = $db->exec('insert into Usuario values(null,"'.mysql_real_escape_string($jsB['uname']).'","'.mysql_real_escape_string($jsB['email']).'",md5("'.mysql_real_escape_string($jsB['password']).'"))');
		} catch (Exception $e) {
			echo '{"R":-2}';
			return;
		}
...
$f3->route('POST /Login',
	function($f3) {
		...
		$jsB = json_decode($Cuerpo,true);
		/////////////
		$R = array_key_exists('uname',mysql_real_escape_string($jsB)) && array_key_exists('password',mysql_real_escape_string($jsB));
		// TODO checar si estan vacio los elementos del json
		if (!$R){
			echo '{"R":-1}';
			return;
		...
		// TODO Control de error de la $DB
		try {
			$R = $db->exec('Select id from  Usuario where uname ="'.mysql_real_escape_string($jsB['uname']).'" and password = md5("'.mysql_real_escape_string($jsB['password']).'");');
		} catch (Exception $e) {
			echo '{"R":-2}';
			return;
		}
		...
		$db->exec('Delete from AccesoToken where id_Usuario = "'.mysql_real_escape_string($R[0]['id']).'";');
		$R = $db->exec('insert into AccesoToken values('.mysql_real_escape_string($R[0]['id']).',"'.mysql_real_escape_string($T).'",now())');
		echo "{\"R\":0,\"D\":\"".$T."\"}";
	}
);
...
$f3->route('POST /Imagen',
	function($f3) {
		...
		try {
			$R = $db->exec('select id_Usuario from AccesoToken where token = "'.mysql_real_escape_string($TKN).'"');
		} catch (Exception $e) {
			echo '{"R":-2}';
			return;
		}
		$id_Usuario = mysql_real_escape_string($R[0]['id_Usuario']);
		file_put_contents('tmp/'.$id_Usuario,base64_decode($jsB['data']));
		$jsB['data'] = '';
		////////////////////////////////////////////////////////
		////////////////////////////////////////////////////////
		// Guardar info del archivo en la base de datos
		$R = $db->exec('insert into Imagen values(null,"'.mysql_real_escape_string($jsB['name']).'","img/",'.mysql_real_escape_string($id_Usuario).');');
		$R = $db->exec('select max(id) as idImagen from Imagen where id_Usuario = '.mysql_real_escape_string($id_Usuario));
		$idImagen = mysql_real_escape_string($R[0]['idImagen']);
		$R = $db->exec('update Imagen set ruta = "img/'.mysql_real_escape_string($idImagen).'.'.mysql_real_escape_string($jsB['ext']).'" where id = '.mysql_real_escape_string($idImagen));
...
$f3->route('POST /Descargar',
	function($f3) {
		...
		try {
			$R = $db->exec('Select name,ruta from  Imagen where id = '.mysql_real_escape_string($idImagen));
		}catch (Exception $e) {
			echo '{"R":-3}';
			return;
		}
